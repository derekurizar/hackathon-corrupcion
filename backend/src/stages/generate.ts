...
import { generateStoryGuarded, type GuardedStoryResult } from '../generation/guardrails.js';
...
import type { CuePoint, Entity } from '../schema/index.js';
...
/** Per-case cue points captured during the audio phase (idea/04 §"Podcast"). */
export interface AudioCaseResult {
  caseKey: string;
  version: number;
...
> {
  ...
  try {
    ...
    const audioResults: AudioCaseResult[] = [];
    // Per-run story cache: populated in the audio phase, reused by publish so
    // each case's story is generated by Claude exactly once (and the audio
    // narration + article share the SAME non-deterministic generation). Empty
    // when the audio phase didn't run → publish generates normally.
    ...
    if (cfg.RUN_AUDIO && bundles.length > 0) {
      ...
      if (voiceEs === '' || voiceEn === '') {
        ...
        for (const bundle of bundles) {
          ...
          const guarded = await generateStoryGuarded(
            client,
            bundle,
            anon.displayNameEs,
            anon.displayNameEn,
          ...
          // Cache BEFORE the `continue` so publish reuses the EXACT fallback
          // decision for skipped-audio cases (else it would regenerate and
          // could diverge from the case that produced no podcast audio).
          storyCache.set(bundle.caseKey, guarded);
          const { story } = guarded;
          ...
          audioResults.push({
            caseKey: bundle.caseKey,
            version: audioVersion,
            podcastCuePoints: out.podcastCuePoints,
          });
        }
    ...
    if (cfg.RUN_PUBLISH) {
      ...
      const pr = await publish({ runId, scope, bundles, storyCache });
    ...
    return {
      ...
      ...(audioResults.length > 0 ? { audioResults } : {}),
    };
  } catch (err) {
