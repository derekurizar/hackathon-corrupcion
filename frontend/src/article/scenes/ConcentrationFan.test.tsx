...
describe('ConcentrationFan', () => {
  it('fixture validates against the synced scene-contract schema', () => {
    expect(SCENES.ConcentrationFan?.schema.safeParse(concentrationFanParams).success).toBe(true);
  });
  ...
  it('renders the caption and a screen-reader supplier table', () => {
    ...
    expect(screen.getAllByText(/Un proveedor concentra/i).length).toBeGreaterThan(0);
    // Supplier names appear in the React Flow node AND the sr-only table.
    expect(screen.getAllByText('Proveedor anónimo A').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('Proveedor anónimo C').length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText('Fixture narration for concentration fan.')).toBeInTheDocument();
  });
});
